...
# File: refresh_docker_compose.md
...
# Modern task runners (alternatives to Make)
...
**Not logging**: Doesn't record descrete events or historical data. 
...
# Reminder: Engage with the course

Act in accordance with DevOps principles. That's one of the metrics that you will be evaluated on. 

1. PR Reminder

    Remember to criticially engage with the course material. 

    This should be easier now that you get the [whoknows_variations](https://github.com/who-knows-inc/EK_DAT_DevOps_2026_Spring/blob/main/00._Course_Material/01._Assignments/04._Sofware_Quality_Linting_CI/02._After/whoknows_variations_continuous_integration.md) assignments.
...
[Mandatory I](https://github.com/who-knows-inc/EK_DAT_DevOps_2026_Spring/blob/main/00._Course_Material/01._Assignments/00._Mandatories/mandatory_I.md)
  ...
  deploy:
    ...
    steps:
      ...
      - name: Transfer Docker compose file to server
        run: |
          scp -i ~/.ssh/ssh_key -o StrictHostKeyChecking=no src/docker-compose.prod.yml $SSH_USER@$SSH_HOST:docker-compose.yml
        env:
            SSH_USER: ${{ secrets.SSH_USER }}
            SSH_HOST: ${{ secrets.SSH_HOST }}
